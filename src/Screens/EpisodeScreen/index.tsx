...
import { GET_ONE_EPISODE } from '../../query/oneEpisode';

export const EpisodeScreen: React.FC = ({ route }) => {
  const { id } = route?.params;
  const { data, loading, error } = useQuery(GET_ONE_EPISODE, {
    variables: {
      id,
    },
  });
  ...
  return (
    <ScrollView>
      <SecondaryHeader title={data.episode.name} />
      <StyledLocationInfo>
        <StyledType>{data.episode.created}</StyledType>
        <StyledName>{data.episode.name}</StyledName>
        <StyledDimension>{data.episode.episode}</StyledDimension>
      </StyledLocationInfo>
      {data.episode.characters.length ? (
        <StyledListWrapper>
          <StyledListTitle>Characters</StyledListTitle>
          <FlatList
        ...
        <StyledName style={{ textAlign: 'center', textTransform: 'uppercase' }}>
          Персонажей нет
        </StyledName>
      )}
    </ScrollView>
